...
// Cam bien Gyro L3GD20 | đk thanh ghi
...
/* USER CODE BEGIN 0 */
/*
 * Cấu hình khởi tạo Cảm biến L3GD20 qua SPI5
 * true nếu tìm thấy cảm biến, false nếu thất bại
 * */
bool GYRO_Init(void) {
	uint8_t tx_data[2];
	uint8_t rx_data[2];

	// 1. Kiem tra WHO_AM_I (Xac nhan ket noi)
	tx_data[0] = L3GD20_WHO_AM_I_ADDR | SPI_READ_BIT; // 0x8F
	tx_data[1] = 0x00; // Dummy byte de tao clock doc

	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_RESET); // Keo CS xuong LOW
	HAL_SPI_TransmitReceive(&hspi5, tx_data, rx_data, 2, HAL_MAX_DELAY);
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET);   // Keo CS len HIGH

	uint8_t who_am_i = rx_data[1];
	if (who_am_i != 0xD4 && who_am_i != 0xD3) {
		return false; // Khong tim thay L3GD20 hoac I3G4250D
	}

	// 2. Bat nguon va truc XYZ (Ghi vao CTRL_REG1)
	tx_data[0] = L3GD20_CTRL_REG1_ADDR; // Ghi thi khong Set bit 0x80
...
/*
 * Doc 6 byte du lieu tu OUT_X_L den OUT_Z_H
 *
 * */
void GYRO_ReadXYZ(int16_t *x, int16_t *y, int16_t *z) {
	// Dia chi thanh ghi dau tien + Lenh doc + Lenh tu tang (Auto-increment)
	uint8_t tx_cmd = L3GD20_OUT_X_L_ADDR | SPI_READ_BIT | SPI_MULTIPLE_BIT; // 0xE8
	uint8_t rx_buf[7] = { 0 }; // Chua 1 byte rac + 6 byte data

	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_RESET);

	// Gui lenh 1 byte
	HAL_SPI_Transmit(&hspi5, &tx_cmd, 1, HAL_MAX_DELAY);
	/* Doc ve 6 byte (khi goi TransmitReceive hay Receive, STM32 tu tao clock) */
	HAL_SPI_Receive(&hspi5, &rx_buf[1], 6, HAL_MAX_DELAY);

	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET);

	// Ep kieu va ghep byte (So co dau bu 2) */
	*x = (int16_t) ((rx_buf[2] << 8) | rx_buf[1]);
	*y = (int16_t) ((rx_buf[4] << 8) | rx_buf[3]);
	*z = (int16_t) ((rx_buf[6] << 8) | rx_buf[5]);
}
/*
 * Ngat nhan UART (Preemption Priority = 1)
 *
 * */
...
/*
 * Ngat Timer 1s (Preemption Priority = 2)
 * */
...
int main(void) {
	...
	MX_USART1_UART_Init();
	/* USER CODE BEGIN 2 */
	// Keo CS len muc cao de cam bien cho lenh
	...
	while (1) {
		...
		// 4. QUAN LY THOI GIAN DEN LED (NON-BLOCKING)
		if (led_green_active) {
